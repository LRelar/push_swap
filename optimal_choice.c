...
/*   Updated: 2019/10/14 20:18:25 by ekedge-w         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	search_fs(t_frame *fr, int min, int max)
{
	...
	while (head != tail )
	{
		if ((head->data >= fr->stages[min]) && (head->data <= fr->stages[max]) && ( fr->fst == NULL))
			fr->fst = head;
		if ((tail->data >= fr->stages[min]) && (tail->data <= fr->stages[max]) && (fr->sec == NULL))
			fr->sec = tail;
...
void calc_com1(t_frame *fr)
{
	...
	if (fr->depth1 <= fr->MEDIAN_B)
	{
		...
		fr->com1[1][1] = fr->depth1;
	}
	else
	{
		fr->com1[1][1] = fr->LEN_B - fr->depth1;
		fr->com1[1][1] = fr->LEN_B - fr->depth1 + 1;
	}
	reverse(fr->com1);
}
...
{
	if ((fr->LEN_A - fr->depth2 - 1) <= fr->MEDIAN_A)
	...
	if (fr->depth2 <= fr->MEDIAN_B)
	{
		...
		fr->com2[1][1] = fr->depth2;
	}
	else
	{
		fr->com2[1][1] = fr->depth2;
		fr->com2[1][0] = fr->depth2 + 1;
	}
	reverse(fr->com2);
}
